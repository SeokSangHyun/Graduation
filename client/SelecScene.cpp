...
void CSelectScene::BuildLightsAndMaterials()
{
...
void CSelectScene::UpdateMaterial(CGameObject *pObject)
{
...
void CSelectScene::SetMaterial(int nIndex, MATERIAL *pMaterial)
{
...
void CSelectScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_ppObjects[0] = new CAirplanePlayer(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, 0);
	m_ppObjects[0]->SetPosition(XMFLOAT3(-80.0f, 370.0f, -50.0f));
	m_ppObjects[0]->Rotate(0.0f, -90.0f, -90.0f);

	m_ppObjects[1] = new CAirplanePlayer(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, 0,1, 1);
	m_ppObjects[1]->SetPosition(XMFLOAT3(85.0f, 370.0f, -50.0f));
	m_ppObjects[1]->Rotate(0.0f, -90.0f, -90.0f);
	m_ppObjects[1]->SetScale(0.25, 0.25, 0.25);

	m_ppObjects[2] = new CBall(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature); 
	m_ppObjects[2]->SetPosition(XMFLOAT3(-80.0f, 370.0f, 40.0f));
	m_ppObjects[2]->Rotate(0.0f, -90.0f, -90.0f);
	...
	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/Image/UI/Charter Select Scene.dds", 0);
	pObjectShader->BuildObjects(pd3dDevice, pd3dCommandList, FRAME_BUFFER_WIDTH, FRAME_BUFFER_HEIGHT, 0, 0, NULL, pTexture);
...
void CSelectScene::ReleaseUploadBuffers()
{
...
ID3D12RootSignature *CSelectScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice) //루트시그니처 두개 쓰는건 오바인가....
{
...
void CSelectScene::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
...
void CSelectScene::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
...
bool CSelectScene::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	return(false);
}

bool CSelectScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
	case WM_KEYDOWN:
		...
		{
			SelectJob = 1;
			m_ppObjects[2]->SetPosition(XMFLOAT3(SelectJob * 85.0f, 370.0f, 40.0f));
			break;
		}
		case VK_LEFT:
		{
			SelectJob = 0;
			m_ppObjects[2]->SetPosition(XMFLOAT3(SelectJob - 80.0f , 370.0f, 40.0f));
			break;
		}
		default:
			break;
...
bool CSelectScene::ProcessInput(UCHAR *pKeysBuffer)
{
	return(false);
}

//따라 보기 추가
void CSelectScene::AnimateObjects(float fTimeElapsed, CCamera *pCCamera)
{
	...
	{
		rot += 0.01f;
		if (m_ppObjects[2]->GetPosition().x == -80)
		{
			m_ppObjects[0]->Animate(fTimeElapsed, pCCamera);
			m_ppObjects[0]->GetMesh()->FBXFrameAdvance(fTimeElapsed);
			m_ppObjects[0]->Rotate(0, rot, 0);
		}
		else {
			m_ppObjects[1]->Animate(fTimeElapsed, pCCamera);
			m_ppObjects[1]->GetMesh()->FBXFrameAdvance(fTimeElapsed);
			m_ppObjects[1]->Rotate(0, rot, 0);
		}
		m_ppObjects[2]->Rotate(0,rot,0);
	}
